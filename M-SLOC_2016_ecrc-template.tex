...
\textcolor{red}{The computational cost of the GCC-PHAT Patterns extraction stage has been calculated in \cite{DoSY07}, and it is now reported here. Denoting with $L$ the size of a frame, the computational cost, in terms of total number of operations per frame, is given by the following processing steps:
\begin{itemize}
...
Although this architecture was designed for image processing, several works in the literature have employed CNNs in the audio field \cite{thomas2014analyzing,mcloughlin2015low}. In this work, we arrange the input data as a 3-D tensor (as described in \secref{sec:TEE}), %(\textbf{ndemap: dove?})),
inspired by what occurs in image processing with RGB channels. Thus, the CNN-SLOC is able to efficiently process adjacent frames and to exploit the temporal evolution of the audio signal \cite{wirn2016-vad}. Up to the authors' knowledge, CNNs have never been employed for indoor speaker localization. % in the scientific literature.
...
%In this work, the estimated Cartesian coordinates are smoothed by using a median filter. The coordinates are smoothed separately since they are assumed independent \cite{vermaak2001nonlinear}. % and different window size have been evaluated for the best performing setups of the different methods.
}
...
\begin{table}[t]
	...
	\caption{Parameters explored during the optimization stages.}
	\label{tab:CNNs}
...
\multicolumn{1}{c|}{44805}             & \multicolumn{1}{c|}{47630}               & \multicolumn{1}{c|}{17580}                    & 12885  \\
\hline                    
\end{tabular}
...
\begin{figure}[t]
	...
	\begin{subfigure}[t]{0.5\textwidth}
					...
					\includegraphics{imgs/boxplot_simulated_living.tikz}
	
		\caption{Living Room.}\label{fig:boxplot_simu-living}
 	 \end{subfigure}%
		\begin{subfigure}[t]{0.5\textwidth}
			\centering
					\includegraphics{imgs/boxplot_simulated_kitchen.tikz}
...
The effect of temporal context is considered in the fourth optimization stage, in which the network size determined after the third stage is kept fixed. %This last stage is compared to the previous in order to highlight the decisive effect of the exploitation of an audio excerpt.   
...
\textcolor{red}{{as reported in \tableref{tab:bestCONF_SIMU}.}}
In particular, the CNN-SLOC 2Rx1N architecture reduces the RMSE by 37\% with respect to the method proposed in a previous work by the authors \cite{vesperini2016sloc}.
...
\begin{figure}[t]
	\centering
	\begin{subfigure}[t]{0.45\textwidth}
	\includegraphics{imgs/boxplot_real_living.tikz}
	\caption{Living Room.}
	\end{subfigure}
	\begin{subfigure}[t]{0.45\textwidth}
	\includegraphics{imgs/boxplot_real_kitchen.tikz}
	\caption{Kitchen.}
	\end{subfigure}	
...
\centering
\begin{subfigure}[t]{0.45\textwidth}
\includegraphics[width=\textwidth]{imgs/REALCNN1N1RLivRoom_Labels.tikz}
\caption{Distribution of the speaker's positions in the ground truth.}\label{fig:distr_real_gt}
\end{subfigure}
\begin{subfigure}[t]{0.45\textwidth}
\includegraphics[width=\textwidth]{imgs/REALCNN1N1RLivRoom_Predictions.tikz}
