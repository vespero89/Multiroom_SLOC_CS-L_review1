...
%%
\textcolor{red}{Observing the results related to the MLP-SLOC \figref{sfig:mlp-real-context}, it can be noticed that increasing the context size up to 19 frames generally reduces the RMSE regardless the value of the stride. Similarly, increasing the value of the stride the RMSE reduces. Indeed, the differences when using stride greater than 3 are more limited. Regarding the performance of the CNN-SLOC (\figref{sfig:cnn-real-context}), the behavior is similar to the one observed for the MLP-SLOC. Here, however the performance improves up to a context of 15 frames. 

A similar behavior can be observed for different values of the stride, where a part from the results related to stride equal to 1, the results are similar. }
