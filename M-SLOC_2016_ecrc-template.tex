...
The main contribution of this work is the development of a completely data-driven approach for Speaker Localization (SLOC) in a multi-room environment. The purpose of the data-driven strategy is to avoid a dedicated fine-tuning of parameters, which is typical and highly specific for the state of the art algorithms. In details, the proposed algorithm is composed of a feature extraction stage and an artificial neural network, composing together the DNN-SLOC.
...
Hence, the GCC-PHAT Patterns are extracted as follows: for each considered microphone pair the CSPCM is computed with a frame size and a hop size respectively equal to 480\,ms and 160\,ms.
...
Commonly, a kernel layer is coupled with a pooling layer in order to assess more robustness against patterns shifts in the processed data. The CNN structure is completed by an MLP layer, whose task is to produce the network response - the estimated ($\chi,\psi$) positions in this case. 
...
After that, Least Mean Square is applied for the generic point $\mathbf{p}=[x,y]$, with the purpose of minimizing the error defined as the sum of the squared distances $D_j(\mathbf{p})$ between $\mathbf{p}$ and the $j$-th DOA:
\begin{equation}
...
\end{equation}
where $\chi$ and $\psi$  are the network outputs, $\chi_{\text{ref}}$ and $\psi_{\text{ref}}$ are the reference coordinates, and $N_{TOT}$ is the total number of frames.
...
Similarly to a classification task, $P_{cor}$ provides a measure of the localization accuracy, since the estimated positions with an RMSE less than 500\,mm represent the correct decisions. These two metrics are averaged over all the available network outputs. 
...
In this case the CNN results more effective with respect to the MLP architecture, and it is able to further reduce the RMSE. In particular, the CNN-2Rx1N architecture reduces the RMSE by 37\% with respect to the method proposed in a previous work by the authors \cite{vesperini2016sloc}.
...
The  results obtained in the investigation of the audio excerpt are reported in \figref{ctx-str-SIMU}, where the different strides $s$ are plotted while varying the temporal context $C$. It can be noticed that a similar trend of performance with respect to temporal resolution values is registered.
...
\begin{figure}[t] 
	...
	\addplot [draw = CadetBlue, mark = *, CadetBlue] coordinates{(0,466.28) (1,440.11) (2,388.49) (3,383.4) (4,383.6) (5,563.75) (6,439.3) (7,379.4)};
	...
	\addplot [draw = Gray, mark =diamond*, Gray] coordinates{(0,412.31) (1,366.82) (2,327.48) (3,330.62) (4,310.63) (5,371.1) (6,350) (7,334.71)};
	\legend{\emph{Strides 1},\emph{Strides 3},\emph{Strides 4},\emph{Strides 5}}
	\end{axis}
	\end{tikzpicture}%
	%
	...
	\caption{Performance trend in the Simulated dataset for different \textit{strides} at growing \textit{context}. The considered room is Kitchen with 1Rx1N configuration, the two DNN are plotted.}
	\label{ctx-str-SIMU}
\end{figure}
...
A particular effort has been directed to the evaluation of a spatial and temporal context, revealing the latter to be extremely decisive. In details, audio coming from one or two rooms has been jointly exploited (1Rx1N, 2Rx1N), while the temporal context has been tested by means of close in time frames. The algorithm implicitly requires an Oracle VAD in order to process only human speech. Results are evaluated on the DIRHA Dataset in comparison with two state-of-the-art algorithm, based respectively on Crosspower Spectrum Phase estimation and on Steered Response Power.
