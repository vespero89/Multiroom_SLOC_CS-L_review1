...
A block diagram of the algorithm based on a CNN is shown in \figref{fig:proposed_method}.  %ndFAB
%%%
%The target positions are scaled between 0 and 1, thus the linear output of the ANN produces always valid coordinates which are simply unscaled to the $\left [mm\right ]$ values at the end of the processing.
%%%
...
Hence, the GCC-PHAT Patterns are extracted as follows: for each considered microphone pair the CSPCM is computed with a frame size of 30\,ms and a hop size of 10\,ms respectively equal to 480 samples and 160 samples at the sample rate of 16\,kHz. %ndFAB
