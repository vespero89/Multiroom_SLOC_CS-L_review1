...
randpts=5000;  %%% J0 in SRC. Depending on the size of the search volume, choose an appropriate value here (Here, 3000 is for a V_{search}= 4m x 1m x 6m) 
...
%% SRC algorithm:
    ...
    function [bestpos,bestsrp,nofes] = src(magiconst,bstart,bend,randpts,npoints,mic_loc,yintpt,row1,efsintp)
        ...
        %minVoxel=1000*10^(-6);  %%%the threshold (subject to change, depending on your data) of the sub-volume,i.e., a (cube-root of 100)-cm cube
        minVoxel=0.05;
        maxFEs=50000;      %%% threshold for the maximum number of functional evaluations (fe's) allowed to evaluate
