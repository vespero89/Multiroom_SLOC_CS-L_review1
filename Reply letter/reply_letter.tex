...
\item \textit{Conclusion: To me, the conclusion is way too optimistic and biased. Two main points to challenge are:}
	...
	\end{enumerate}
 \textbf{Reply}: Thank you for your comment. Regarding the first remark, the CSP-SLOC algorithm requires the knowledge of the room geometry (in particular, the distance between the microphone arrays and the opposite wall), and it requires tuning of the exponential window scaling factor $\alpha$ and of the averaging weighting coefficient $\mu$ for cepstral dereverberation. The SRP-SLOC algorithm requires the knowledge of the room geometry and of the positions of the microphones, and tuning the Stochastic Region Contraction parameters $J_0$ and $N_0$. The hyperparameters of the proposed approach are related to the architecture of the network (considering the CNN-SLOC, the number kernels, the number of layers, the number of units per layer), the context size and the stride, and the choice of the microphone pairs. Regarding the microphone pairs, as shown in Figure 6 and Figure 9, the dependence on the chosen pair is modest.
 
%The proposed approach performs speaker localization directly on GCC-PHAT patterns data, and provided that enough data for 
% Regarding the generalisation capabilities of the proposed approach,
